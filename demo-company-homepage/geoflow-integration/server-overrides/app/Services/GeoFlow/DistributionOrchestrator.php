...
class DistributionOrchestrator
{
    ...
    public function enqueueForArticle(int|Article $article, string $action = 'publish'): void
    {
        try {
            ...
            // Published articles without a task still enter the configured Tongzhuo publisher queue.
            $assistantChannels = DistributionChannel::query()
                ->whereIn('channel_type', ['desktop_publisher', 'wechatsync_manual'])
                ->where('status', 'active')
                ->get();
            $channels = $channels
                ->concat($assistantChannels)
                ->unique('id')
                ->values();
