...
assert.deepEqual({ projectId: seed.projectId, industryTemplate: seed.industryTemplate, demo: seed.demo }, { projectId: "machinery-demo", industryTemplate: "machinery", demo: true });
...
const runtime = createSiteRuntime({ database, workspaceId: "default", projectId: seed.projectId, projectSeedKey: seed.key, staticRoot: root, host: "127.0.0.1", port: 0, baseUrl: "https://machinery.example.invalid", flushIntervalMs: 60_000, logger: { info() {}, warn() {}, error() {} } });
...
try {
  ...
  assert.equal(database.connection.prepare("SELECT COUNT(*) AS count FROM site_contact_leads WHERE workspace_id = ? AND project_id = ?").get("default", seed.projectId).count, 1);
  assert.equal(database.connection.prepare("SELECT COUNT(*) AS count FROM site_contact_leads WHERE workspace_id != ? OR project_id != ?").get("default", seed.projectId).count, 0);
