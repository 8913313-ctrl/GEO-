...
try {
  ...
  assert.equal(result.response.status, 200); assert.equal(result.body.data.version.id, unreviewedVersion.id); assert.equal(result.body.data.version.reviewStatus, "pending", "the exact freshly saved version must enter human review"); remoteArticle = result.body.data.article;
  result = await request(`/api/v1/content/tasks/${task.id}/request-changes`, { method: "POST", headers: reviewerAuth, body: JSON.stringify({ versionId: unreviewedVersion.id, expectedRevision: remoteArticle.revision, note: "请补充证据适用边界。" }) });
  assert.equal(result.response.status, 200, JSON.stringify(result.body));
  assert.equal(result.body.data.version.reviewStatus, "changes_requested");
  assert.equal(result.body.data.article.status, "changes_requested");
  assert.equal(result.body.data.task.status, "changes_requested");
  assert.equal(result.body.data.version.reviews.at(-1).action, "changes_requested");
  assert.equal(result.body.data.version.reviews.at(-1).note, "请补充证据适用边界。");
  result = await request(`/api/v1/content/articles/${encodeURIComponent(article.id)}`, { headers: { Cookie: cookie } });
  assert.equal(result.response.status, 200, JSON.stringify(result.body));
  assert.equal(result.body.data.article.currentVersion.reviewStatus, "changes_requested");
  assert.equal(result.body.data.article.currentVersion.reviews.at(-1).note, "请补充证据适用边界。", "article refresh must retain the latest change-request note");
  remoteArticle = result.body.data.article;
  result = await request(`/api/v1/content/tasks/${task.id}/versions`, { method: "POST", headers: auth, body: JSON.stringify({ articleId: article.id, expectedRevision: remoteArticle.revision, baseVersionId: unreviewedVersion.id, title: article.title, contentHtml: "<p>已补充证据适用边界的新正文。</p>", evidence }) });
  assert.equal(result.response.status, 201, JSON.stringify(result.body));
  assert.equal(result.body.data.version.reviewStatus, "draft", "a returned version must be editable as a new draft");
  remoteArticle = result.body.data.article;
  result = await request(`/api/v1/content/tasks/${task.id}/can-publish`, { headers: { Cookie: cookie } });
