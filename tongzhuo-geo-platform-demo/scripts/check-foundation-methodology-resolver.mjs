...
try {
  ...
  for (const [workspaceId, planId, name] of [
    ["tenant_tongzhuo_geo", "PLAN-TONGZHUO", "桐灼 GEO 内容计划"],
    ["tenant_building_materials", "PLAN-BUILDING", "建材企业内容计划"]
  ]) {
  ...
  const tongzhuo = resolver.resolveArticleContext({ workspaceId: "tenant_tongzhuo_geo", planId: "PLAN-TONGZHUO", customerQuestion: "企业做 GEO 内容时怎样引用官网事实？", contentType: "深度文章" });
  const building = resolver.resolveArticleContext({ workspaceId: "tenant_building_materials", planId: "PLAN-BUILDING", customerQuestion: "采购保温建材时应该如何比较产品参数？", contentType: "采购指南" });
  assert.equal(tongzhuo.versionId, building.versionId, "different tenants must reuse the same published global methodology version");
  ...
  assert.ok(!JSON.stringify(tongzhuo).includes("建材企业"), "methodology context must not contain another tenant's project data");
  assert.ok(!JSON.stringify(building).includes("桐灼"), "methodology context must not contain another tenant's project data");
  assert.throws(() => resolver.resolveArticleContext({ workspaceId: "tenant_building_materials", planId: "PLAN-TONGZHUO", customerQuestion: "测试问题是什么？" }), /Content plan not found/);
} finally {
  database.close();
  await rm(temporaryDirectory, { recursive: true, force: true });
}

console.log("Per-task methodology fragment selection, shared global version, and tenant boundary checks passed.");
