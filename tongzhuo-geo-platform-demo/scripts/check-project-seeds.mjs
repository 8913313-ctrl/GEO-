...
try {
  ...
  const blankStore = new SiteCmsStore(database, { workspaceId: "deployment-blank-customer", projectSeedKey: "" });
  ...
  const tongzhuoStore = new SiteCmsStore(database, { workspaceId: "deployment_tongzhuo_geo", projectSeedKey: "tongzhuo-geo" });
  ...
  assert.deepEqual({ projectId: buildingSeed.projectId, industryTemplate: buildingSeed.industryTemplate, demo: buildingSeed.demo }, { projectId: "building-materials-demo", industryTemplate: "building-materials", demo: true });
  const buildingStore = new SiteCmsStore(database, { workspaceId: "building-materials-demo", projectSeedKey: buildingSeed.key });
  ...
  const energy = new SiteCmsStore(database, { workspaceId: "energy-demo", projectSeedKey: energySeed.key }).publication().snapshot;
  ...
  const beauty = new SiteCmsStore(database, { workspaceId: "beauty-demo", projectSeedKey: beautySeed.key }).publication().snapshot;
  ...
  assert.throws(
    () => new PublicSiteStore({ database, workspaceId: "deployment-blank-customer", projectSeedKey: "" }).snapshot(),
    (error) => error?.code === "SITE_CMS_NOT_PUBLISHED",
    "a fresh customer must have no public snapshot before first publication"
  );

  const tongzhuoPublic = new PublicSiteStore({ database, workspaceId: "deployment_tongzhuo_geo", projectSeedKey: "tongzhuo-geo" }).snapshot();
