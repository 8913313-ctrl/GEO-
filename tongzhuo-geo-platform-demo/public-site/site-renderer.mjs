...
const FRONTEND_NAV = Object.freeze([
  { label: "首页", path: "/" }, { label: "产品与服务", path: "/services/" }, { label: "服务案例", path: "/cases/" },
  { label: "行业资讯", path: "/insights/" }, { label: "问题地图", path: "/problem-map/" }, { label: "关于我们", path: "/about/" }, { label: "联系我们", path: "/contact/" }
]);
...
function pageTitle(site, title = "") {
  return title ? `${title}｜${site.siteName}` : site.siteName;
}
...
function navigation(site, active = "") {
  ...
  const visibleItems = site.frontendDemo
    ...
    : items;
  return `<header class="site-header"><div class="shell nav"><a class="brand" href="/" aria-label="${escapeHtml(site.siteName)}首页"><span class="brand-mark" aria-hidden="true"><i></i><i></i></span><span><b>${escapeHtml(site.siteName)}</b><small>TRUSTED BUSINESS SOURCE</small></span></a><nav class="nav-links" aria-label="主导航">${visibleItems.map((item) => `<a${activePath === normalize(item.path) ? " class=\"active\" aria-current=\"page\"" : ""} href="${escapeHtml(item.path)}">${escapeHtml(item.label)}</a>`).join("")}</nav><div class="nav-actions"><a class="nav-cta" href="/contact/">${escapeHtml(site.cta || "预约业务咨询")}</a><button class="menu-toggle" type="button" aria-label="打开导航" aria-expanded="false" aria-controls="mobile-navigation"><span></span><span></span><span></span></button></div></div><nav id="mobile-navigation" class="mobile-navigation" aria-label="移动端导航">${visibleItems.map((item) => `<a${activePath === normalize(item.path) ? " class=\"active\"" : ""} href="${escapeHtml(item.path)}">${escapeHtml(item.label)}</a>`).join("")}<a class="mobile-cta" href="/contact/">${escapeHtml(site.cta || "预约业务咨询")}</a></nav></header>`;
}

function footer(site) {
  const contact = site.contact || {};
  return `<footer class="site-footer"><div class="shell footer-main"><div class="footer-brand"><a class="brand" href="/"><span class="brand-mark" aria-hidden="true"><i></i><i></i></span><span><b>${escapeHtml(site.siteName)}</b><small>ENTERPRISE SOURCE</small></span></a><p>${escapeHtml(site.description || DEFAULT_DESCRIPTION)}</p>${contact.serviceArea || contact.industryRegion ? `<span class="footer-meta">${escapeHtml([contact.industryRegion, contact.serviceArea].filter(Boolean).join(" · "))}</span>` : ""}</div><div class="footer-col"><strong>业务</strong><a href="/services/">产品与服务</a><a href="/insights/">行业资讯</a><a href="/contact/">业务咨询</a></div><div class="footer-col"><strong>信源</strong><a href="/sitemap.xml">站点地图</a><a href="/llms.txt">AI 内容索引</a><a href="/feed.xml">RSS 订阅</a></div><div class="footer-col"><strong>企业</strong><a href="/about/">关于我们</a>${contact.phone ? `<a href="tel:${escapeHtml(contact.phone)}">${escapeHtml(contact.phone)}</a>` : ""}${contact.email ? `<a href="mailto:${escapeHtml(contact.email)}">${escapeHtml(contact.email)}</a>` : ""}</div></div><div class="shell footer-bottom"><span>© ${new Date().getFullYear()} ${escapeHtml(site.companyName || site.siteName)}</span><span>正式内容经过企业知识与人工审核后发布</span></div></footer>`;
}

function documentShell({ site, origin, pathname, title, description, active, schemaExtra = [], body, robots = "index,follow,max-image-preview:large,max-snippet:-1", feed = true, preview = false, assetBase = "/site-assets", headLinks = [], openGraphType = "website", headMeta = [] }) {
  const canonical = absoluteUrl(origin, pathname);
  ...
  return `<!doctype html><html lang="zh-CN" style="--brand:${escapeHtml(primary)}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${escapeHtml(resolvedTitle)}</title><meta name="description" content="${escapeHtml(metaDescription)}"><meta name="robots" content="${escapeHtml(preview ? "noindex,nofollow,noarchive" : robots)}"><meta name="author" content="${escapeHtml(site.companyName || site.siteName)}"><link rel="canonical" href="${escapeHtml(canonical)}">${extraLinks}${feed && !preview ? `<link rel="alternate" type="application/rss+xml" title="${escapeHtml(site.siteName)}行业资讯" href="/feed.xml">` : ""}<meta property="og:title" content="${escapeHtml(resolvedTitle)}"><meta property="og:description" content="${escapeHtml(metaDescription)}"><meta property="og:type" content="${escapeHtml(openGraphType)}"><meta property="og:url" content="${escapeHtml(canonical)}"><meta property="og:site_name" content="${escapeHtml(site.siteName)}"><meta property="og:locale" content="zh_CN"><meta name="twitter:card" content="summary"><meta name="twitter:title" content="${escapeHtml(resolvedTitle)}"><meta name="twitter:description" content="${escapeHtml(metaDescription)}">${extraMeta}<meta name="theme-color" content="${escapeHtml(primary)}"><link rel="stylesheet" href="${escapeHtml(assetBase)}/site.css?v=20260804-site-refresh5"><script type="application/ld+json">${safeJsonLd(schema)}</script></head><body${preview ? ' class="is-preview"' : ""}><a class="skip-link" href="#main">跳到正文</a>${preview ? `<div class="preview-bar"><span>CMS 草稿预览</span><b>该页面尚未影响正式官网</b></div>` : ""}${navigation(site, active)}<main id="main">${body}</main>${footer(site)}<script src="${escapeHtml(assetBase)}/site.js?v=20260727-visual5" defer></script></body></html>`;
}
...
function renderHomePage({ site, page, articles, categories, origin, preview = false, assetBase = "/site-assets" }) {
  ...
  const brandName = escapeHtml(site.siteName || "桐灼科技");
...
function renderServicesPage({ site, page, origin, preview = false, assetBase = "/site-assets" }) {
  const hero = moduleOf(site, page.id, "hero", preview);
  ...
  const serviceTags = `<div class="hero-tag-row">${FRONTEND_SERVICES.slice(0, 6).map((service) => `<span>${escapeHtml(service.title)}</span>`).join("")}</div>`;
...
function renderCasesPage({ site, page, origin, preview = false, assetBase = "/site-assets" }) {
  const FRONTEND_CASES = frontendCases(site, preview);
  ...
  const renderedBody = body.replace(/<div class="case-filter" data-case-filter>[\s\S]*?<\/div><div class="case-grid case-grid-wide">/, `${filters}<div class="case-grid case-grid-wide">`)
    .replace("第一版先用典型场景展示我们如何从企业事实、客户问题和内容运营开始。", `当前展示 ${FRONTEND_CASES.length} 个经过审核的公开案例与典型场景。`)
...
function renderContactPage({ site, page, origin, preview = false, assetBase = "/site-assets" }) {
  const contact = site.contact || {};
...
export function renderProblemPage({ site, problem, group, articles = [], origin, preview = false, assetBase = "/site-assets" }) {
...
export function renderFixedPage({ site, page, articles = [], categories = [], origin, preview = false, assetBase = "/site-assets" }) {
...
function articleMeta(article) {
  return `<div class="blog-meta"><span>${escapeHtml(article.categoryName || "行业观点")}</span><span>${escapeHtml(article.author || "企业内容团队")}</span><span>${Math.max(1, Math.ceil(plainText(article.contentText || article.contentHtml).length / 500))}分钟阅读</span></div>`;
}
...
export function renderArticlePage({ site, article, origin, relatedArticles = [], compatibility = false }) {
  ...
  const body = `<header class="article-hero"><div class="shell"><span class="kicker">${escapeHtml(article.categoryName || "行业观点")}</span><h1>${escapeHtml(article.title)}</h1>${article.excerpt ? `<p>${escapeHtml(article.excerpt)}</p>` : ""}<div class="article-meta"><span>作者：${escapeHtml(article.author || site.siteName)}</span>${publishedMeta}${modifiedMeta}<span>预计阅读：${Math.max(1, Math.ceil(plainText(article.contentText || article.contentHtml).length / 500))}分钟</span></div></div></header><article class="shell article-layout" id="article" data-content-article-id="${escapeHtml(article.id)}">${tableOfContents}<div class="prose">${article.excerpt ? `<div class="answer-box"><strong>内容摘要</strong><p>${escapeHtml(article.excerpt)}</p></div>` : ""}${contentHtml}${article.tags?.length ? `<div class="source-note">主题：${escapeHtml(article.tags.join("、"))}</div>` : ""}<div class="source-note">本文由${escapeHtml(article.author || site.siteName)}发布，内容来自企业内容工作台的已审核版本。${updateNote}</div></div></article>${related}<section class="contact-band contact-band-v2 article-contact-band"><div class="shell contact-grid"><div class="contact-copy"><span class="eyebrow">Build Your Source</span><h2>让企业知识成为客户和 AI 可以理解的可信信源</h2><p>${escapeHtml(site.description || DEFAULT_DESCRIPTION)}</p></div><div class="contact-form"><strong class="contact-form-title">${escapeHtml(site.cta || "了解服务")}</strong><p class="contact-form-description">查看服务详情，或提交与本文相关的业务问题。</p><a class="button ink" href="/contact/">联系我们 <span class="arrow">→</span></a></div></div></section>`;
  ...
  const renderedBody = withoutRepeatedExcerpt
    .replace("内容来自企业内容工作台的已审核版本", "内容来自企业审核通过的正式版本");
  const headMeta = [
...
export function renderLlms({ site, articles, origin, full = false }) {
  ...
  if (insightsPage) lines.push(`- [行业资讯](${absoluteUrl(origin, "/insights/")}): 仅列出已审核、已发布的正式文章。`);
  lines.push(
