...
- 43280 运营页面读取中央 API 的客户、实例、任务、账本、经营分析和上游状态；真实原始回答仍由客户实例拉取并落库。收款核验、发票登记、导出、设置保存、实例暂停/恢复/吊销与密钥轮换、失败重试、人工退款和交付死信重投均写入真实 API/审计账本，不再在浏览器修改演示数组。
...
默认地址：`http://127.0.0.1:43280/#dashboard`
