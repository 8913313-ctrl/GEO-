...
@php
    ...
    $contentOps = is_array($contentOps ?? null) ? $contentOps : [];
    $serviceLineCards = is_array($serviceLineCards ?? null) ? $serviceLineCards : [];
    $contentNextActions = is_array($contentNextActions ?? null) ? $contentNextActions : [];
    $articleQuality = is_array($articleQuality ?? null) ? $articleQuality : [];
@endphp

@section('content')
    <div class="px-4 sm:px-0">
        {{-- content_ops_console: article_production website_publish distribution_queue geo_quality_gate service_line_matrix --}}
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ $pageTitle }}</h1>
                <p class="mt-1 text-sm text-gray-600">{{ $isTrashView ? __('admin.articles.trash.subtitle') : '围绕GEO优化、短视频运营、企业AI落地，把问题机会生产成可被官网、AI和分发平台引用的证据型内容。' }}</p>
            </div>
        ...
        @else
        <div class="mb-8 grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(360px,0.55fr)]">
            <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-100 px-6 py-5">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            <div class="text-xs font-black tracking-[0.18em] text-blue-600">CONTENT OPERATIONS</div>
                            <h2 class="mt-2 text-xl font-black text-slate-950">内容运营台</h2>
                            <p class="mt-1 max-w-3xl text-sm leading-6 text-slate-500">文章不是孤立发布，必须完成“问题机会 -> 证据型文章 -> 官网行业资讯 -> 发布助手 -> AI采样复测”的闭环。</p>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('admin.geo-opportunities.index') }}" class="inline-flex items-center rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-black text-blue-700 hover:bg-blue-100">
                                <i data-lucide="radar" class="mr-1.5 h-4 w-4"></i>问题机会
                            </a>
                            <a href="{{ route('admin.distribution.jobs') }}" class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-50">
                                <i data-lucide="send" class="mr-1.5 h-4 w-4"></i>分发队列
                            </a>
                            <a href="{{ route('admin.geo-answer-tests.index') }}" class="inline-flex items-center rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700 hover:bg-emerald-100">
                                <i data-lucide="messages-square" class="mr-1.5 h-4 w-4"></i>AI复测
                            </a>
                        </div>
                    </div>
                </div>
                <div class="grid gap-px bg-slate-100 sm:grid-cols-2 xl:grid-cols-4">
                    <div class="bg-white p-5">
                        <div class="text-xs font-bold text-slate-500">AI生成草稿</div>
                        <div class="mt-2 text-3xl font-black text-slate-950">{{ (int) ($contentOps['ai_generated'] ?? 0) }}</div>
                    </div>
                    <div class="bg-white p-5">
                        <div class="text-xs font-bold text-slate-500">需补SEO/GEO</div>
                        <div class="mt-2 text-3xl font-black text-amber-600">{{ (int) ($contentOps['needs_seo'] ?? 0) }}</div>
                    </div>
                    <div class="bg-white p-5">
                        <div class="text-xs font-bold text-slate-500">未进入分发</div>
                        <div class="mt-2 text-3xl font-black text-blue-600">{{ (int) ($contentOps['published_without_distribution'] ?? 0) }}</div>
                    </div>
                    <div class="bg-white p-5">
                        <div class="text-xs font-bold text-slate-500">分发异常</div>
                        <div class="mt-2 text-3xl font-black text-red-600">{{ (int) ($contentOps['distribution_failed'] ?? 0) }}</div>
                    </div>
                </div>
                <div class="grid gap-3 p-5 md:grid-cols-3">
                    @foreach($serviceLineCards as $line)
                        @php
                            $tone = (string) ($line['tone'] ?? 'blue');
                        ...
                        <div class="rounded-xl border {{ $toneClass }} p-4">
                            <div class="text-sm font-black">{{ $line['label'] }}</div>
                            <div class="mt-3 flex items-end gap-2">
                                <span class="text-3xl font-black">{{ (int) ($line['published'] ?? 0) }}</span>
                                <span class="pb-1 text-xs font-bold opacity-80">已发布 / {{ (int) ($line['total'] ?? 0) }} 总内容</span>
                            </div>
                            <div class="mt-2 text-xs font-bold opacity-80">草稿 {{ (int) ($line['draft'] ?? 0) }} 篇，持续补齐问题地图覆盖。</div>
                        </div>
                    @endforeach
                </div>
            </section>

            <aside class="rounded-2xl border border-slate-200 bg-slate-950 p-5 text-white shadow-sm">
                <div class="text-xs font-black tracking-[0.18em] text-cyan-300">NEXT ACTIONS</div>
                <h2 class="mt-2 text-xl font-black">今日内容动作</h2>
                <div class="mt-5 space-y-3">
                    @foreach($contentNextActions as $action)
                        <div class="rounded-xl border border-white/10 bg-white/5 p-4">
                            <div class="flex items-center gap-2 font-black">
                                <i data-lucide="{{ $action['icon'] ?? 'circle-dot' }}" class="h-4 w-4 text-cyan-300"></i>
                                {{ $action['title'] ?? '' }}
                            </div>
                            <p class="mt-2 text-sm leading-6 text-slate-300">{{ $action['desc'] ?? '' }}</p>
                        </div>
                    @endforeach
                </div>
            </aside>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        @foreach($articles as $article)
                            @php
                                ...
                                $quality = $articleQuality[(int) $article->id] ?? null;
                                $qualityLevel = is_array($quality) ? (string) ($quality['level'] ?? 'weak') : 'weak';
                                $qualityClass = match($qualityLevel) {
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @if(is_array($quality))
                                        <div class="mt-2 flex flex-wrap items-center gap-1.5">
                                            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 {{ $qualityClass }}">
                                                <i data-lucide="shield-check" class="mr-1 h-3 w-3"></i>
                                                GEO质量 {{ (int) ($quality['score'] ?? 0) }}分
                                            </span>
                                            <span class="text-xs text-gray-500">{{ $quality['next_action'] ?? '' }}</span>
                                        </div>
                                    @endif
                                </td>
                                    ...
                                    @else
                                        <div class="flex items-center space-x-2">
                                            ...
                                            @if((string) $article->status === 'published')
                                                <form method="POST" action="{{ route('admin.articles.dispatch', ['articleId' => (int) $article->id]) }}" class="inline">
                                                    @csrf
                                                    <button type="submit" class="text-blue-600 hover:text-blue-800" title="送入发布助手">
