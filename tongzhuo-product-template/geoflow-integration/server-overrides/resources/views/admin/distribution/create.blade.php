...
@php($channelType = old('channel_type', 'desktop_publisher'))
...
@section('content')
                ...
                <form method="POST" action="{{ route('admin.distribution.store') }}" class="space-y-6">
                    ...
                    <fieldset class="rounded-lg border border-gray-200 bg-gray-50 p-4">
                        ...
                        <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
                            <label class="flex cursor-pointer gap-3 rounded-md border border-cyan-200 bg-white p-4 hover:border-cyan-400">
                                <input type="radio" name="channel_type" value="desktop_publisher" class="mt-1 text-cyan-600 focus:ring-cyan-500" @checked($channelType === 'desktop_publisher')>
                                <span>
                                    <span class="block text-sm font-semibold text-gray-900">{{ __('admin.distribution.channel_type.desktop_publisher') }}</span>
                                    <span class="mt-1 block text-sm text-gray-600">{{ __('admin.distribution.channel_type.desktop_publisher_desc') }}</span>
                                </span>
                            </label>
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input id="endpoint_url" name="endpoint_url" type="text" value="{{ old('endpoint_url') }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="{{ __('admin.distribution.placeholder.endpoint_url') }}">
                            <p class="mt-1 text-xs text-gray-500">{{ __('admin.distribution.help.endpoint_url') }}</p>
                        </div>
                    </div>

                    <div data-channel-type-panel="desktop_publisher" @class(['space-y-6', 'hidden' => $channelType !== 'desktop_publisher'])>
                        @include('admin.distribution._wechatsync-platforms')
                    </div>
                    ...
                    <div data-channel-type-panel="wordpress_rest" @class(['rounded-lg border border-blue-100 bg-blue-50 p-5', 'hidden' => $channelType !== 'wordpress_rest'])>
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-6 md:grid-cols-3">
                            ...
                            <div>
                                ...
                                <input id="wordpress_fixed_category" name="wordpress_fixed_category" type="text" value="{{ old('wordpress_fixed_category') }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="1 鎴?News">
                            </div>
                            <div>
    ...
    <script>
        function syncDistributionChannelType(type) {
            document.querySelectorAll('[data-channel-type-panel]').forEach(function (panel) {
                panel.classList.toggle('hidden', panel.dataset.channelTypePanel !== type);
            });
            const endpointInput = document.getElementById('endpoint_url');
            if (endpointInput) {
                endpointInput.required = ! ['desktop_publisher', 'wechatsync_manual'].includes(type);
            }
        }
        ...
        document.addEventListener('change', function (event) {
            ...
            syncDistributionChannelType(event.target.value);
        });

        syncDistributionChannelType(@js($channelType));
    </script>
@endsection
