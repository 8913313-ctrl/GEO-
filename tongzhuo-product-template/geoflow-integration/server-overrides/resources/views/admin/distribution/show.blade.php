...
@php($wechatsyncConfig = $channel->resolvedWechatSyncConfig())
@php($healthCheckUrl = $channel->isWordPressRest() ? $channel->wordpressRestBaseUrl().'/wp/v2/users/me?context=edit' : ($channel->isLocalPublisher() ? __('admin.distribution.wechatsync.health_check_label') : rtrim((string) $channel->endpoint_url, '/').'/geoflow-agent/v1/health'))
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="rounded-lg bg-white p-6 shadow lg:col-span-2">
                ...
                <dl class="mt-5 grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
                    ...
                    @elseif ($channel->isLocalPublisher())
                        <div>
                            <dt class="text-gray-500">{{ __('admin.distribution.wechatsync.platforms_title') }}</dt>
                            <dd class="mt-1 font-medium text-gray-900">{{ implode(' / ', array_map(fn ($platform) => __('admin.distribution.wechatsync.platforms.'.$platform), $wechatsyncConfig['wechatsync_platforms'])) }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500">{{ __('admin.distribution.wechatsync.publish_mode') }}</dt>
                            <dd class="mt-1 font-medium text-gray-900">{{ __('admin.distribution.wechatsync.publish_mode_draft') }}</dd>
                        </div>
                    @else
                        <div>
            ...
            <div class="rounded-lg bg-white p-6 shadow">
                ...
                @if ($channel->activeSecret)
                    ...
                    @endif
                @elseif ($channel->isLocalPublisher())
                    <div class="mt-5 rounded-md border border-cyan-100 bg-cyan-50 px-3 py-3 text-sm leading-6 text-cyan-900">
                        {{ __('admin.distribution.wechatsync.secret_hint') }}
                    </div>
                @endif
            </div>
        </div>
            ...
            <div class="rounded-lg bg-white p-6 shadow">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <div class="rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700">
                        <span class="font-medium">{{ __('admin.distribution.detail.agent_package') }}锛?/span>
        ...
        @elseif ($channel->isLocalPublisher())
            <div class="rounded-lg bg-white p-6 shadow">
                <div class="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                    <div class="max-w-3xl">
                        <h2 class="text-lg font-medium text-gray-900">{{ __('admin.distribution.wechatsync.guide_title') }}</h2>
                        <p class="mt-2 text-sm leading-6 text-gray-600">{{ __('admin.distribution.wechatsync.guide_desc') }}</p>
                    </div>
                    <div class="rounded-md border border-cyan-100 bg-cyan-50 px-4 py-3 text-sm text-cyan-900">
                        <div class="font-medium">{{ __('admin.distribution.wechatsync.selected_platforms') }}</div>
                        <div class="mt-1">{{ implode(' / ', array_map(fn ($platform) => __('admin.distribution.wechatsync.platforms.'.$platform), $wechatsyncConfig['wechatsync_platforms'])) }}</div>
                    </div>
                </div>
                <ol class="mt-6 grid grid-cols-1 gap-4 text-sm text-gray-700 md:grid-cols-2 xl:grid-cols-4">
                    <li class="flex gap-3">
                        <span class="flex h-7 w-7 flex-none items-center justify-center rounded-full bg-cyan-100 text-xs font-semibold text-cyan-700">1</span>
                        <span>{{ __('admin.distribution.wechatsync.guide_step_channel') }}</span>
                    </li>
                    <li class="flex gap-3">
                        <span class="flex h-7 w-7 flex-none items-center justify-center rounded-full bg-cyan-100 text-xs font-semibold text-cyan-700">2</span>
                        <span>{{ __('admin.distribution.wechatsync.guide_step_queue') }}</span>
                    </li>
                    <li class="flex gap-3">
                        <span class="flex h-7 w-7 flex-none items-center justify-center rounded-full bg-cyan-100 text-xs font-semibold text-cyan-700">3</span>
                        <span>{{ __('admin.distribution.wechatsync.guide_step_download') }}</span>
                    </li>
                    <li class="flex gap-3">
                        <span class="flex h-7 w-7 flex-none items-center justify-center rounded-full bg-cyan-100 text-xs font-semibold text-cyan-700">4</span>
                        <span>{{ __('admin.distribution.wechatsync.guide_step_publish') }}</span>
                    </li>
                </ol>
            </div>
        @else
                            ...
                            <div class="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
                                <div class="min-w-0">
                                    ...
                                    <div class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500">
                                        <span class="whitespace-nowrap">{{ __('admin.distribution.field.time') }}锛歿{ $log->created_at?->format('Y-m-d H:i') }}</span>
                                        <span class="whitespace-nowrap">{{ __('admin.distribution.field.event') }}锛歿{ $log->event ?: __('admin.common.none') }}</span>
                                        <span class="whitespace-nowrap">{{ $logLevelLabel }}</span>
                                    </div>
                                </div>
                                <div class="min-w-0 text-gray-600 lg:max-w-xl lg:text-right">
                                    <span class="text-gray-400">{{ __('admin.distribution.field.article') }}锛?/span>
