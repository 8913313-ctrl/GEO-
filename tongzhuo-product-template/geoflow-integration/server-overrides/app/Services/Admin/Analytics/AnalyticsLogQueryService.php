...
class AnalyticsLogQueryService
{
    ...
    private const SYSTEM_USER_AGENT_PATTERNS = [
        'wget', 'curl/', 'python-requests', 'go-http-client', 'kube-probe',
        'healthcheck', 'uptime', 'censys', 'palo alto', 'infrawatch',
        'modatscanner', 'internetmeasurement', 'zgrab', 'masscan', 'nikto',
    ];
    ...
    private function baseLocalQuery(AnalyticsFilter $filter): Builder
    {
        ...
        $this->excludeSystemTraffic($query);
    ...
    private function excludeSystemTraffic(Builder $query): void
    {
        $this->whereNoPattern($query, self::SYSTEM_USER_AGENT_PATTERNS);
    }
