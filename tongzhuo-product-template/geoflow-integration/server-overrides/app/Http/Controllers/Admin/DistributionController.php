...
class DistributionController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $stats = [
            ...
            'paused' => DistributionChannel::query()->where('status', 'paused')->count(),
            'pending' => ArticleDistribution::query()->whereIn('status', ['queued', 'sending'])->count(),
            'synced' => ArticleDistribution::query()->where('status', 'synced')->count(),
            'failed' => ArticleDistribution::query()->where('status', 'failed')->count(),
            'local_channels' => DistributionChannel::query()->whereIn('channel_type', ['desktop_publisher', 'wechatsync_manual'])->count(),
            'online_devices' => PublisherDevice::query()->whereNull('disabled_at')->where('last_seen_at', '>=', now()->subMinutes(2))->count(),
            'attention' => ArticleDistribution::query()
                ->whereIn('status', ['queued', 'sending', 'failed'])
                ->count(),
        ];
        $recentJobs = ArticleDistribution::query()
            ->with(['article:id,title,slug,status', 'channel:id,name,domain,channel_type'])
            ->orderByDesc('updated_at')
            ->limit(6)
            ->get();
        $channelTypeSummary = $channels
            ->groupBy(fn (DistributionChannel $channel): string => $channel->channelType())
        ...
        return view('admin.distribution.index', [
            ...
            'channelTypeSummary' => $channelTypeSummary,
            'logs' => $logs,
        ]);
    }
    ...
    public function update(Request $request, int $channelId): RedirectResponse
    {
        ...
        if ($channel->activeSecret && ! $channel->isLocalPublisher()) {
            try {
    ...
    public function jobs(Request $request): View
    {
        ...
        $channels = DistributionChannel::query()
            ...
            ->get();
        $jobStats = [
            'queued' => ArticleDistribution::query()->where('status', 'queued')->count(),
            'sending' => ArticleDistribution::query()->where('status', 'sending')->count(),
            'synced' => ArticleDistribution::query()->where('status', 'synced')->count(),
            'failed' => ArticleDistribution::query()->where('status', 'failed')->count(),
            'online_devices' => PublisherDevice::query()->whereNull('disabled_at')->where('last_seen_at', '>=', now()->subMinutes(2))->count(),
            'active_channels' => DistributionChannel::query()->where('status', 'active')->count(),
        ];
    ...
    public function rotateSecret(int $channelId): RedirectResponse
    {
        ...
        if (! $channel->isGeoFlowAgent()) {
            return back()->withErrors(__('admin.distribution.message.secret_rotation_only_for_agent'));
        }
    ...
    public function revealSecret(Request $request, int $channelId): RedirectResponse
    {
        ...
        if (! $channel->isGeoFlowAgent()) {
            return back()->withErrors(__('admin.distribution.message.target_package_only_for_agent'));
        }
    ...
    public function downloadWechatSyncPackage(int $distributionId): StreamedResponse|RedirectResponse
    {
        ...
        if (! $distribution->channel->isLocalPublisher()) {
            return back()->withErrors(__('admin.distribution.wechatsync.package_only_for_manual'));
        }
        if ((string) $distribution->action === 'delete') {
            return back()->withErrors(__('admin.distribution.wechatsync.package_not_available_for_delete'));
        }

        $package = $this->wechatsyncPublishPackageBuilder->build($distribution);
    ...
    public function retry(int $distributionId): RedirectResponse
    {
        ...
        if (is_array($remoteMeta['publisher_assistant'] ?? null)) {
            $remoteMeta['previous_publisher_assistant'] = $remoteMeta['publisher_assistant'];
            $remoteMeta['publisher_assistant'] = [
                'state' => 'queued',
                'worker_id' => null,
    ...
    public function confirmPublisherResult(Request $request, int $distributionId): RedirectResponse
    {
        $payload = $request->validate([
            'platform' => ['nullable', 'string', 'max:120'],
            'remote_url' => ['nullable', 'url', 'max:500'],
            'note' => ['nullable', 'string', 'max:1000'],
        ]);

        return $this->recordPublisherOperatorResult(
            $distributionId,
            'published',
            filled($payload['remote_url'] ?? null) ? (string) $payload['remote_url'] : null,
            filled($payload['note'] ?? null) ? (string) $payload['note'] : '运营人员已确认平台发布完成。',
            filled($payload['platform'] ?? null) ? (string) $payload['platform'] : null,
        );
    }

    public function failPublisherResult(Request $request, int $distributionId): RedirectResponse
    {
        $payload = $request->validate([
            'platform' => ['nullable', 'string', 'max:120'],
    ...
    private function recordPublisherOperatorResult(int $distributionId, string $state, ?string $remoteUrl, string $message, ?string $platform): RedirectResponse
    {
        $distribution = ArticleDistribution::query()
            ->with(['article', 'channel'])
            ->whereKey($distributionId)
        ...
        if (! $distribution || ! $distribution->article || ! $distribution->channel || ! $distribution->channel->isLocalPublisher()) {
            return back()->withErrors(__('admin.distribution.message.job_not_found'));
        }

        $remoteMeta = is_array($distribution->remote_meta) ? $distribution->remote_meta : [];
        $assistant = is_array($remoteMeta['publisher_assistant'] ?? null) ? $remoteMeta['publisher_assistant'] : [];
        $platformResults = is_array($assistant['platform_results'] ?? null) ? $assistant['platform_results'] : [];
        $platformKey = $platform !== null && $platform !== '' ? $platform : 'manual_confirmation';
        $now = now()->toIso8601String();
        $adminId = auth('admin')->id();

        $platformResults[$platformKey] = array_replace(
            is_array($platformResults[$platformKey] ?? null) ? $platformResults[$platformKey] : [],
            [
        ...
        foreach ($platformResults as $result) {
            ...
            $resultState = (string) ($result['state'] ?? 'unknown');
            $stateCounts[$resultState] = ($stateCounts[$resultState] ?? 0) + 1;
        }

        $confirmations = is_array($assistant['operator_confirmations'] ?? null) ? $assistant['operator_confirmations'] : [];
        $confirmations[] = [
            'state' => $state,
            'platform' => $platformKey,
            'remote_url' => $remoteUrl,
        ...
        $assistant = array_replace($assistant, [
            'state' => $state,
            'updated_at' => $now,
            'completed_at' => $state === 'published' ? $now : ($assistant['completed_at'] ?? null),
            'last_error' => $state === 'failed' ? $message : null,
            'platform_results' => $platformResults,
            'state_summary' => [
                'state' => $state,
                'operator_action' => $state === 'published' ? 'none' : 'operator_inspect_failed_platforms',
                'state_counts' => $stateCounts,
                'failed_platforms' => array_values(array_keys(array_filter($platformResults, fn ($result): bool => is_array($result) && (string) ($result['state'] ?? '') === 'failed'))),
                'total_platforms' => count($platformResults),
            ],
            'next_operator_action' => $state === 'published' ? 'none' : 'operator_inspect_failed_platforms',
            'operator_confirmations' => $confirmations,
        ]);
        $remoteMeta['publisher_assistant'] = $assistant;

        $distribution->forceFill([
            'status' => $state === 'published' ? 'synced' : 'failed',
            'remote_url' => $remoteUrl ?: $distribution->remote_url,
            'remote_meta' => $remoteMeta,
            'last_error_message' => $state === 'failed' ? $message : null,
        ])->save();

        $this->distributionOrchestrator->log(
            $state === 'published' ? 'info' : 'error',
            $state === 'published' ? '运营人员已确认本地发布完成' : '运营人员记录本地发布失败',
            (int) $distribution->distribution_channel_id,
            (int) $distribution->id,
            (int) $distribution->article_id,
            [
                'event' => $state === 'published' ? 'publisher_assistant.operator_confirmed' : 'publisher_assistant.operator_failed',
                'state' => $state,
                'platform' => $platformKey,
                'remote_url' => $remoteUrl,
                'message' => $message,
        ...
        return back()->with('message', $state === 'published'
            ? '已记录人工确认发布结果。'
            : '已记录人工发布失败结果。');
    }
    ...
    {
        $wechatsyncPlatformRule = 'in:'.implode(',', DistributionChannel::wechatSyncPlatformKeys());
        $payload = $request->validate([
            'name' => ['required', 'string', 'max:120'],
            'domain' => ['required', 'string', 'max:255'],
            'endpoint_url' => ['nullable', 'string', 'max:500'],
            'channel_type' => ['nullable', 'string', 'in:geoflow_agent,wordpress_rest,desktop_publisher,wechatsync_manual'],
            'front_mode' => ['nullable', 'string', 'in:static,rewrite'],
            'template_key' => ['nullable', 'string', 'max:120'],
            'status' => ['required', 'string', 'in:active,paused'],
            'description' => ['nullable', 'string', 'max:1000'],
            'wechatsync_platforms' => ['nullable', 'array'],
            'wechatsync_platforms.*' => ['string', $wechatsyncPlatformRule],
        ...
        $endpointUrl = $this->normalizeEndpointUrl((string) ($payload['endpoint_url'] ?? ''));
        if (in_array($payload['channel_type'], ['desktop_publisher', 'wechatsync_manual'], true) && $endpointUrl === '') {
            $endpointUrl = $payload['channel_type'] === 'desktop_publisher'
                ? 'https://local.tongzhuo-publisher.invalid'
                : 'https://local.wechatsync.invalid';
        }
        $payload['endpoint_url'] = $endpointUrl;

        if (! in_array($payload['channel_type'], ['desktop_publisher', 'wechatsync_manual'], true) && ! $this->isValidHttpEndpoint($endpointUrl)) {
            throw ValidationException::withMessages([
                'endpoint_url' => __('admin.distribution.validation.endpoint_url'),
            ]);
        }
        if (in_array($payload['channel_type'], ['desktop_publisher', 'wechatsync_manual'], true) && $endpointUrl !== '' && ! $this->isValidHttpEndpoint($endpointUrl)) {
            throw ValidationException::withMessages([
                'endpoint_url' => __('admin.distribution.validation.endpoint_url'),
            ]);
        }
    ...
    private function normalizeChannelSiteSettings(array $payload, ?DistributionChannel $channel = null): array
    {
        ...
        $defaults = $channel?->resolvedSiteSettings() ?? [
            ...
            'site_subtitle' => '',
            'site_description' => 'GEOFlow 自动分发和管理的目标站点。',
            'site_keywords' => '',
            'copyright_info' => '漏 '.date('Y').' '.($defaultName !== '' ? $defaultName : 'GEOFlow Target Site'),
            'site_logo' => '',
            'site_favicon' => '',
    ...
    {
        if (in_array((string) ($payload['channel_type'] ?? 'geoflow_agent'), ['desktop_publisher', 'wechatsync_manual'], true)) {
            $defaults = $channel?->resolvedWechatSyncConfig() ?? [
                'wechatsync_platforms' => ['wechat_mp', 'zhihu', 'toutiao', 'juejin', 'csdn'],
                'wechatsync_default_mode' => 'draft',
                'wechatsync_content_format' => 'markdown_and_html',
            ];
            $platforms = $payload['wechatsync_platforms'] ?? $defaults['wechatsync_platforms'];
            if (! is_array($platforms)) {
                $platforms = [];
            }
            $platforms = array_values(array_intersect(array_map('strval', $platforms), DistributionChannel::wechatSyncPlatformKeys()));
            if ($platforms === []) {
                $platforms = $defaults['wechatsync_platforms'];
            }

            return [
                'wechatsync_platforms' => $platforms,
                'wechatsync_default_mode' => 'draft',
                'wechatsync_content_format' => 'markdown_and_html',
            ];
        }
