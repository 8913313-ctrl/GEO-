...
use App\Models\Task;
use App\Services\GeoGrowth\ArticleGeoQualityScorer;
...
class ArticleController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        return view('admin.articles.index', [
            ...
            'contentOps' => $isTrashView ? [] : $this->loadContentOps(),
            'serviceLineCards' => $isTrashView ? [] : $this->loadServiceLineCards(),
            'contentNextActions' => $isTrashView ? [] : $this->loadContentNextActions(),
            'articleQuality' => $isTrashView ? [] : $this->qualitySnapshot($articles),
        ]);
    }
    ...
    /**
     * 将已发布文章重新送入当前启用的发布助手和分发渠道。
     */
    public function dispatchToPublisher(int $articleId): RedirectResponse
    {
        $article = Article::query()->whereKey($articleId)->firstOrFail();
     ...
     */
    private function loadContentOps(): array
    {
        $publishedWithoutDistribution = Article::query()
            ->where('status', 'published')
            ->whereDoesntHave('distributions')
            ->count();

        return [
            'ai_generated' => Article::query()->where('is_ai_generated', 1)->count(),
            'featured' => Article::query()->where('is_featured', 1)->count(),
            'needs_seo' => Article::query()
                ->where('status', 'published')
                ...
                ->count(),
            'published_without_distribution' => $publishedWithoutDistribution,
            'distribution_queued' => ArticleDistribution::query()->whereIn('status', ['queued', 'sending'])->count(),
            'distribution_synced' => ArticleDistribution::query()->where('status', 'synced')->count(),
            'distribution_failed' => ArticleDistribution::query()->where('status', 'failed')->count(),
        ];
    }
     ...
     */
    private function loadServiceLineCards(): array
    {
        return collect([
            ['key' => 'geo', 'label' => 'GEO优化', 'keywords' => ['GEO', 'AI搜索', 'AI抓取', '可见性'], 'tone' => 'blue'],
            ['key' => 'short_video', 'label' => '短视频运营', 'keywords' => ['短视频', '抖音', '视频号', '直播'], 'tone' => 'rose'],
            ['key' => 'enterprise_ai', 'label' => '企业AI落地', 'keywords' => ['AI落地', '智能体', '知识库', '自动化'], 'tone' => 'emerald'],
        ])->map(function (array $line): array {
            $query = Article::query();
            $keywords = $line['keywords'];
            $query->where(function ($subQuery) use ($keywords): void {
     ...
     */
    private function loadContentNextActions(): array
    {
        $ops = $this->loadContentOps();
        $actions = [];
        if (($ops['needs_seo'] ?? 0) > 0) {
            $actions[] = ['icon' => 'search-check', 'title' => '补齐SEO/GEO摘要', 'desc' => '已发布文章中仍有内容缺少关键词或摘要，会影响官网和AI抓取质量。'];
        }
        if (($ops['published_without_distribution'] ?? 0) > 0) {
            $actions[] = ['icon' => 'send', 'title' => '送入发布助手', 'desc' => '有已发布官网文章还没有进入分发队列，需要同步到发布设备。'];
        }
        if (($ops['distribution_failed'] ?? 0) > 0) {
            $actions[] = ['icon' => 'circle-alert', 'title' => '处理分发失败', 'desc' => '发布助手或平台回写存在失败任务，需要在分发管理中重试或人工确认。'];
        }
        if ($actions === []) {
            $actions[] = ['icon' => 'circle-check', 'title' => '内容闭环正常', 'desc' => '当前文章生产、官网发布和分发队列没有明显阻塞，继续按问题机会池生产内容。'];
        }
     ...
     * @return array<int,array<string,mixed>>
     */
    private function qualitySnapshot(LengthAwarePaginator $articles): array
    {
        return collect($articles->items())
            ->filter(fn ($article): bool => $article instanceof Article)
            ->mapWithKeys(fn (Article $article): array => [
                (int) $article->id => $this->qualityScorer->score($article),
            ])
            ->all();
    }
