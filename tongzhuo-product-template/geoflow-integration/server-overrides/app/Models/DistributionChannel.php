...
class DistributionChannel extends Model
{
    ...
    public function resolvedSiteSettings(): array
    {
        ...
        return [
            ...
            'site_description' => trim((string) ($stored['site_description'] ?? 'GEOFlow 自动分发和管理的目标站点。')),
            'site_keywords' => trim((string) ($stored['site_keywords'] ?? '')),
            'copyright_info' => trim((string) ($stored['copyright_info'] ?? '漏 '.date('Y').' '.($siteName !== '' ? $siteName : 'GEOFlow Target Site'))),
    ...
    public function channelType(): string
    {
        ...
        return in_array($type, ['geoflow_agent', 'wordpress_rest', 'desktop_publisher', 'wechatsync_manual'], true) ? $type : 'geoflow_agent';
    }
    ...
    public function isWechatSyncManual(): bool
    {
        return $this->channelType() === 'wechatsync_manual';
    }

    public function isDesktopPublisher(): bool
    {
        return $this->channelType() === 'desktop_publisher';
    }

    public function isLocalPublisher(): bool
    {
        return in_array($this->channelType(), ['desktop_publisher', 'wechatsync_manual'], true);
    }
    ...
    /**
     * @return array{
     *   wechatsync_platforms:list<string>,
     *   wechatsync_default_mode:string,
     *   wechatsync_content_format:string
     * }
     */
    public function resolvedWechatSyncConfig(): array
    {
        $stored = is_array($this->channel_config) ? $this->channel_config : [];
        $allowedPlatforms = self::wechatSyncPlatformKeys();
        $platforms = $stored['wechatsync_platforms'] ?? ['wechat_mp', 'zhihu', 'toutiao', 'juejin', 'csdn'];
        if (! is_array($platforms)) {
            $platforms = [];
        }

        $platforms = array_values(array_intersect(array_map('strval', $platforms), $allowedPlatforms));
        if ($platforms === []) {
            $platforms = ['wechat_mp', 'zhihu', 'toutiao', 'juejin', 'csdn'];
        }
     ...
     * @return list<string>
     */
    public static function wechatSyncPlatformKeys(): array
    {
        return [
            'wechat_mp',
            'zhihu',
